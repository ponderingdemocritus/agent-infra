...
class SeededRandom {
  ...
  selectSubset<T>(arr: T[], minCount: number = 1, maxCount?: number): T[] {
    ...
    const count = this.nextInt(
      minCount,
      maxCount !== undefined
        ? Math.min(maxCount, arr.length) + 1
        : arr.length + 1
    );
...
const RISK_APPETITES: BehavioralTendencies["risk_appetite"][] = [
  "High",
  "Medium",
  "Low",
];
...
const VERBOSITY_LEVELS: CommunicationStyle["verbosity"][] = [
  "Verbose",
  "Moderate",
  "Terse",
  "Silent",
];
const FORMALITY_LEVELS: CommunicationStyle["formality"][] = [
  "Formal",
  "Neutral",
  "Informal",
  "Crude",
...
const FOCUS_LEVELS: InitialSkillBias["combat_focus"][] = [
  "Very High",
  "High",
  "Medium",
  "Low",
...
function generateName(traits: NumericTraits, random: SeededRandom): string {
  ...
  const hexSnippet = () =>
    random.nextInt(0x100, 0xffff).toString(16).toUpperCase();
  ...
  switch (patternId) {
    ...
    case 3:
      ...
      return `${prefix}${random.select(CRYPTO_TERMS)}${random.nextInt(
        10,
        100
      )}`;
    case 4:
...
// 2. If you haven't already, install ts-node: npm install -g ts-node
